					...
					<table class='tableau'>
						<thead>
							<tr>
								...
								<th>&nbsp;</th>
								<th>Phase</th>
                                <th>{#Niveau#}<br><img width="12" src="../img/b_help.png" title="Importance de la phase dans le classement géléral. Par exemple :
                                     ...
                                     <br><br>
                                     Importance of the phase in the géléral ranking. For example :
                                      <br> The first groups are always at level 1,
                                      <br> - The final is always at the highest level,
                                      <br> - The semifinals will have a higher level than the game for the 5th place,
                                      <br> even if they are played before."></th>
								{if $profile <= 3}
                                <th>{#Tour#}<br><img width="12" src="../img/b_help.png" title="Etape de la phase dans le schéma du système de jeu. Par exemple :
                                         ...
                                         <br><br>
                                         Round of the phase in the game system chart. For example:
                                          <br> - The first groups are always at round 1,
                                          <br> - The final is always at the highest round,
                                          <br> - Several different level phases can be at the same round
                                          <br> (the final may be at the same round as the 3rd place phase)"></th>
                                <th>{#Equipes#}<br><img width="12" src="../img/b_help.png"></th>
                                {/if}
								<th>Type</th>
								<th>{#Nom#}</th>
								<th>{#Date#}</th>
							...
							{section name=i loop=$arrayJournees} 
								<tr class='{cycle values="impair,pair"}'>
									{if $modeEvenement eq '1'}
										{if $arrayJournees[i].Autorisation eq true && $profile <= 4 && $AuthModif == 'O'}
											...
											<td class="color{$arrayJournees[i].Publication|default:'N'}2">
												<img class="publiJournee" data-valeur="{$arrayJournees[i].Publication}" data-id="{$arrayJournees[i].Id}" height="25" src="../img/oeil2{$arrayJournees[i].Publication|default:'N'}.gif" alt="{#Publier#}" />
											</td>
											<td align="left">{$arrayJournees[i].Id}</td> 
											<td width=70>
												<a href="#" onclick="ParamJournee({$arrayJournees[i].Id});"><img height="16" src="../img/glyphicons-31-pencil.png" alt="{#Editer#}" title="{#Editer#}" border="0"></a>
												...
												<a href='GestionJournee.php?idJournee={$arrayJournees[i].Id}'><img height="20" src="../img/b_match.png" alt="{#Matchs_de_la_journee#} {$arrayJournees[i].Id}" title="{#Matchs_de_la_journee#} {$arrayJournees[i].Id}" border="0"></a>
											</td>
										{else}
											{if $profile <= 3 && $AuthModif == 'O'}
												<td>&nbsp;</td>
											{/if}
											<td class="color{$arrayJournees[i].Publication|default:'N'}2">
												<img height="25" src="../img/oeil2{$arrayJournees[i].Publication|default:'N'}.gif" alt="{#Publier#}" title="{#Publier#}" border="0">
											</td>
											<td align="left">{$arrayJournees[i].Id}</td> 
											<td align="left">
												<a href='GestionJournee.php?idJournee={$arrayJournees[i].Id}'>
													<img align="absbottom" height="20" src="../img/b_match.png" alt="{#Matchs_de_la_journee#} {$arrayJournees[i].Id}" title="{#Matchs_de_la_journee#} {$arrayJournees[i].Id}" border="0">
												</a> 
											</td>
										{/if}
									{elseif $arrayJournees[i].Autorisation eq true && $profile <= 4 && $AuthModif == 'O'}
										...
										{/if}
										<td class="color{$arrayJournees[i].Publication|default:'N'}2">
											<img height="25" src="../img/oeil2{$arrayJournees[i].Publication|default:'N'}.gif" alt="{#Publier#}" title="{#Publier#}">
										</td>
										<td align="left">{$arrayJournees[i].Id}</td> 
										<td class="rouge">
